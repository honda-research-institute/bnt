...
num_input_feature = 1;
num_intents = 3;
input_data = csvread('sumo/30/data_300_normal.csv');
input_label= csvread('sumo/30/label_300.csv');
...
hnodes = 1;
data = cell(ss, train_size);
for i=1:train_size
    data{hnodes, i} = hidden_data_train{i};
    data{onodes, i} = obs_data_train{i};
end
% build HMM
bnet = build_HMM(num_features, X, ss, onodes);
Y = 5;
% learn the parameters of HMM using Maximum Likelihood
[transmat, initState] = transmat_train_observed(hidden_data_train, 3);
obsmat = zeros(X,Y);

for i = 1: train_size
   l = hidden_data_train_overall(1,i);
   for h= 1:Y
        obsmat(l,h) = obsmat(l,h)  + length(find(obs_data_train{i}==h));
   end
end
obsmat = normalize(obsmat,2);
obsmat(1,1) = obsmat(3,1);
obsmat(1,4) = obsmat(1,4)-0.0652;
...
%             obs_data{i}(1:length(feat_vector), time_index) = feat_vector;
    ...
    if hidden_data{i}(1,end) == 1
        hidden_data_overall(1,i) = 1;
    elseif  hidden_data{i}(1,end) == 2
