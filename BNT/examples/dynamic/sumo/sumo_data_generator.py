...
np.random.seed(0)
num_run = 1000
scenes = ['left_turn', 'straight', 'right_turn']
step_size = 0.1
noise_std = 0.8
intent_dict = {'left_turn': {'A':1, 'P':2, 'L':3}, 'straight' : {'A':1, 'P':2, 'L':3}, 'right_turn': {'A':4, 'P':5, 'L':6}}
# data_file = open('data_'+str(num_run)+'.csv','wb')
data_file = open('50/data_'+str(num_run)+'.csv','wb')
data_file_normal = open('50/data_'+str(num_run)+'_normal.csv','wb')
data_file_truth = open('50/data_truth_'+str(num_run)+'.csv','wb')
label_file = open('50/label_'+str(num_run)+'.csv','wb')
...
x_intr = 200.0
y_intr = 200.0
start_prediction = 150 #x_intr-twice_intr

min_v,max_v = 10.0, 30.0
min_vx, max_vx = 0.0, 30.0
min_x, max_x = start_prediction-x_intr, twice_intr
min_y,max_y = -twice_intr, twice_intr
min_heading, max_heading = -1.57, 1.57

for i in range(num_run):
    x_true, x_meas, x_pred_normal, labels, ts = [], [], [], [], []
    ekf = EKF.EKF()
    step = 0
    max_step = 1000
    l_steps, l_v = [], []
    ...
    while step < max_step:
        ...
        if not traci.vehicle.getIDList():
            ...
            for carID in traci.vehicle.getIDList():
                ...
                yaw = (angle - angle_prev)/step_size
                v_prev = v
                v = traci.vehicle.getSpeed(carID)
                vx = v * math.cos(angle)
                vy = v * math.sin(angle)
                acc = traci.vehicle.getAccel(carID) # (v-v_prev)/step_size

                if (start_prediction) < x < (x_intr + twice_intr) and (y_intr - twice_intr) < y < (y_intr + twice_intr):

                    cur_x = [x, y, vx, vy, v, acc, angle, yaw]

                    # Add noise to current measurements
                    cur_x_meas = cur_x + np.random.normal(0, noise_std, len(cur_x))
                    cur_x_meas[0] = cur_x_meas[0] - x_intr
                    cur_x_meas[1] = cur_x_meas[1] - y_intr
                    cur_x_meas[2] = cur_x_meas[4] * math.cos(angle)
                    cur_x_meas[3] = cur_x_meas[4] * math.sin(angle)
                    cur_x_meas[5:] = cur_x[5:]
                    ...
                    x_norm, y_norm, vx_norm, vy_norm, v_norm, heading_norm =\
                    normalize_feature(cur_x_meas[0], min_x, max_x, -1, 1), normalize_feature(cur_x_meas[1], min_y, max_y, -1, 1), \
                    normalize_feature(cur_x_meas[2], min_vx, max_vx, -1, 1),normalize_feature(cur_x_meas[3], min_vx, max_vx, -1, 1), \
                    normalize_feature(cur_x_meas[4], min_v, max_v, -1, 1), normalize_feature(cur_x_meas[6], min_heading, max_heading, -1, 1)

                    x_pred_normal.append (np.array([x_norm, y_norm, vx_norm, vy_norm, v_norm, acc, heading_norm, yaw]))
    ...
    traci.close()
    x_meas, x_true, x_pred_normal, labels = \
        np.array(x_meas), np.array(x_true), np.array(x_pred_normal), np.array(labels)
    ts =np.array(ts)

    # ekf_plots()

    # write the dataframe to a tab-separated CSV file
    x_meas = x_meas.flatten()
    x_meas.tofile(data_file, sep=',')
    data_file.write('\n')
